    ...
    internal class Program
    {
        ...
        private static long RunCore()
        {
            using (var channel = CreateChannel())
            {
                ...
                static void Throw() => throw new InvalidOperationException("Unable to format " + nameof(HCForwardPerItemRequest.ItemId));

                var itemRequests = HandCranked.SlabAllocator<HCForwardPerItemRequest>.Rent(BatchSize);
                for (int i = 0; i < BatchSize; ++i)
                {
                    ...
                    itemRequests.Span[i] = new HandCranked.HCForwardPerItemRequest(itemId, itemContext);
                }
                ...
                var request = new HCForwardRequest(traceId: Guid.NewGuid().ToString("N").AsMemory(), itemRequests, requestContextInfo);
